...
class report_performance_employee_atasan1_xls(models.AbstractModel):
	_name = 'report.dym_performance_appraisal.report_employee_atasan1_xls'
	...
	def generate_xlsx_report(self, workbook, data, lines):
		...
		if tipe_report == 'form_a':
			...
			query="""
				...
				where  a.state in ('employee_answer','atasan1_answer') """ + where_department + """ """ + where_periode + """  order by dep.name,he.name
					"""
		else:
			...
			query= """ select he.name as karyawan,he.barcode as nik,dep.loc as cabang,dep.name as department_name,
				...
				left join hr_department dep on dep.id = he.department_id 
				where  b.state in ('employee_answer','atasan1_answer') """ + where_department + """ """ + where_periode + """  order by dep.name,he.name
			"""
		...
		if tipe_report == 'form_b':
			...
			sheet.write(row, 0, nama_sheet + ' Employee Answered to Atasan 1 Answered', bold)
