...
class sign_wizard(models.TransientModel):
    _name = "sign.wizard"

    signature = fields.Binary(string="Digital Signature", copy=False)
    ...
    def btn_continue(self):
        if self.function_name:
            ...
            result = getattr(self.env[self.model_name].search([('id','=',self.data_id)]).with_context(okey=True, signature=self.signature), self.function_name)
