...
class stockMoveLine(models.Model):
    ...
    inventory_qty_done = fields.Float(compute='_compute_inventory_qty_done', string='Pergerakan di Inventory', store=True)
    
    @api.depends('qty_done','location_id','location_id.usage','location_dest_id','location_dest_id.usage')
    def _compute_inventory_qty_done(self):
        for record in self:
            inventory_qty_done = 0
            if record.qty_done:
                if (record.location_id.usage in ('internal','transit')) and (record.location_dest_id.usage not in ('internal','transit')):
                    inventory_qty_done = record.qty_done * -1
                elif (record.location_id.usage not in ('internal','transit')) and (record.location_dest_id.usage in ('internal','transit')):
                    inventory_qty_done = record.qty_done
                # else:
                #     inventory_qty_done = record.qty_done

            record.inventory_qty_done = inventory_qty_done
    ...
    def _action_done(self):
        ...
        if self.picking_id.date_done:
            mls_todo.with_context(bypass_reservation_update=True).write({
                ...
                'date': self.picking_id.date_done, # rubah disini doang
            })
        else:
            mls_todo.with_context(bypass_reservation_update=True).write({
                'product_uom_qty': 0.00,
